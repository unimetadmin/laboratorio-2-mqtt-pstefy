...
def on_message(client, userdata, message):
    ...
    if("tanque" in message.topic):
        if("nivel_agua" in (json.loads(message.payload))):
            query = 'INSERT INTO public.tanque("dateTime", nivel_agua) VALUES (%s, %s);'
            data = ((json.loads(message.payload))['dateTime'], int((json.loads(message.payload))['nivel_agua']))
            select2(query,data)
...
def main():
    client = paho.mqtt.client.Client(client_id='sus_bano', clean_session=False)
